<svelte:head>
	<title>Licensing | openH2</title>
	<meta
		name="description"
		content="How openH2 is licensed: hardware designs under CERN-OHL-W v2, written content under CC BY-SA 4.0."
	/>
</svelte:head>

<section class="mx-auto max-w-3xl px-4">
	<h1 class="text-3xl font-bold tracking-tight md:text-4xl">Licensing</h1>
	<p class="text-muted-foreground mt-4">
		Different kinds of work call for different licenses, so here is exactly what applies to what.
	</p>

	<div class="mt-10 space-y-10">
		<div>
			<h2 class="text-xl font-semibold">Hardware designs: CERN-OHL-W v2</h2>
			<p class="text-muted-foreground mt-2">
				Schematics, parts lists, and build plans are licensed under the
				<a
					href="https://ohwr.org/cern_ohl_w_v2.txt"
					target="_blank"
					rel="noopener noreferrer"
					class="link">CERN Open Hardware Licence v2 (Weakly Reciprocal)</a
				>. Build from these designs, modify them, even sell what you build. If you improve the
				designs themselves, publish your changes under the same terms so the improvements flow back
				to everyone.
			</p>
		</div>

		<div>
			<h2 class="text-xl font-semibold">Written content: CC BY-SA 4.0</h2>
			<p class="text-muted-foreground mt-2">
				Articles, research notes, documentation, and the other written material and files published
				here are licensed under
				<a
					href="https://creativecommons.org/licenses/by-sa/4.0/"
					target="_blank"
					rel="noopener noreferrer"
					class="link">Creative Commons Attribution-ShareAlike 4.0</a
				>. Share and adapt freely, with attribution to openH2.org, and derivative works must stay
				under the same license.
			</p>
		</div>
	</div>

	<p class="text-muted-foreground mt-10 text-sm">
		This page is a plain-language summary, not legal advice; the linked license texts are what
		actually govern.
	</p>
</section>
